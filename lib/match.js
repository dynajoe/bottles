...
Match.prototype.is_game_over = function () {
   return this.is_stopped || this.bots.length === 0 || this.bots.length === 1 || (this.config.max_ticks !== 0 && this.ticks > this.config.max_ticks);
};

Match.prototype.clone = function () {
   var match = new Match(this.config);
   for (var id in this.brains) {
      match.add_bot(this.brains[id]);
   }
   return match;
};
