...
Match.prototype.tick_bots = function (done) {
   ...
   this.bots.forEach(function (bot) {
      if (bot.health <= 0 || bot.waiting_on_command) 
         return;
      ...
      var timeout = setTimeout(function () {
         bot.timed_out = true;
         bot.waiting_on_command = false;
         bot.health -= this.config.bot_timeout_health_loss;
         bot.timeouts++;
         decrement();
      }.bind(this), this.config.bot_timeout);

      setImmediate(function () {
         var tick_requested_at = new Date();
         bot.waiting_on_command = true;
