...
Match.prototype.tick_shells = function () {
   ...
   this.shells.forEach(function (s) {
      ...
      for (var b in this.bots) {
         ...
         if (!is_hit && s.previous_position) {
            is_hit = util.detect_collision(s.previous_position, s.position, bot.position, this.config.bot_radius);
         }
...
Match.prototype.tick_commands = function (commands) {
   commands.forEach(function (c) {
      ...
      if (util.is_number(cmd.fire_power))
         bot.fire_power = util.bound_number(cmd.fire_power, 0, this.config.max_fire_power);
      
      if (util.is_number(cmd.speed))
         bot.speed = util.bound_number(cmd.speed, 0, this.config.max_bot_speed);
