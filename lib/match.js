...
Match.prototype.tick_shells = function () {
   this.shells.forEach(function (s) {
      ...
      for (b in this.bots) {
         ...
         if (util.is_within_range(bot.position, s.position, this.config.bot_radius)) {
...
Match.prototype.tick_commands = function (commands) {
   commands.forEach(function (c) {
      ...
      if (bot.fire_power > 0 && bot.energy > 0) {
         ...
         var shell = { position: { x: bot.position.x, y: bot.position.y }, fire_power: bot.fire_power, heading: bot.turret_heading, speed: bot.fire_power };
