...
Match.prototype.initialize = function () {
   ...
   this.bots.forEach(function (b) {
      b.name = b.name;
      b.position = { x: util.round(this.random(this.config.arena.width)), y: util.round(this.random(this.config.arena.height)) };      
...
Match.prototype.tick_shells = function () {
   ...
   this.shells.forEach(function (s) {
      ...
      for (var b in this.bots) {
         ...
         if (bot.name !== s.owner && util.is_within_range(bot.position, s.position, this.config.bot_radius)) {
...
Match.prototype.is_game_over = function () {
   return this.bots.length === 0 || this.bots.length === 1 || (this.config.max_ticks !== 0 && this.ticks > this.config.max_ticks);
};
