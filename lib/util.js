...
(function () {
   ...
   var closest_point = module.exports.closest_point = function (a, b, c) {
      var ac = new Vector(c).subtract(a);
      var ab = new Vector(b).subtract(a);
      var proj_mag = ac.dot(ab.normalize());

      if (proj_mag < 0) 
         return a;

      if (proj_mag * proj_mag > ab.magnitude_squared()) 
         return b;

      return ac.project(ab).add(a);
   };

   var detect_collision = module.exports.detect_collision = function (previous_pos, current_pos, target_pos, target_radius) {
      var v = new Vector(current_pos);
      var radius_squared = target_radius * target_radius;
      
      if (v.equals(previous_pos))
         return v.distance_squared(target_pos) <= radius_squared;

      var closest = closest_point(previous_pos, current_pos, target_pos);
      var mag = new Vector(target_pos).subtract(closest).magnitude_squared();
      return mag <= radius_squared;
   };
